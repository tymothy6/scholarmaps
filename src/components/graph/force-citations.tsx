...
export default function CitationGraph({
  ...
  const nonSeedGraphNodes = graphData.nodes.filter(
    (node) => node.id !== seedPaperId,
  );

  const minCitationCount = Math.min(
    ...graphData.nodes.map((node) => node.val2),
  );
  ...
  const { q1, q2, q3 } = calculateQuartiles(
    nonSeedGraphNodes.map((node) => node.val),
  );
  ...
  const valToColor = scaleLinear(
    [q1, q2, q3, maxVal],
    ["white", "lightblue", "steelblue", "lightgreen"],
  );

  const borderWidthScale = scaleLinear(
    ...
    <div className="flex flex-col gap-4 items-end">
      <div className="flex justify-between items-center w-full">
        <p className="text-sm text-muted-foreground">
          {nonSeedGraphNodes.length} influential citations found
        </p>
        <GraphLegend
      ...
      <Card ref={containerRef} className="relative w-full h-full min-w-0">
        <ForceGraph2D
          ...
          nodeCanvasObject={(node, ctx, globalScale) => {
            ...
            const fillColor: string = isSeedNode
              ? "#F5F5F5"
              : (valToColor(node.val as number) as unknown as string);
            ...
            const borderColor =
              resolvedTheme === "dark" ? "lightgray" : "black";
              ...
              <TooltipTrigger asChild>
                <div className="absolute top-4 left-4 px-4 py-2 text-primary-foreground bg-primary rounded-sm border-primary shadow-sm">
                  ...
                  <p className="text-sm text-primary-foreground">
                    {hoverNode.val6} ({hoverNode.val3})
                  </p>
                  <p className="text-sm text-primary-foreground">
                    Influential citation count:{" "}
                    <span className="font-mono">{hoverNode.val}</span>
                  </p>
                  <p className="text-sm text-primary-foreground">
                    Citation count:{" "}
                    <span className="font-mono">{hoverNode.val2}</span>
                  </p>
                  <p className="text-sm text-primary-foreground">
                    Reference count:{" "}
                    <span className="font-mono">{hoverNode.val4}</span>
                  </p>
                </div>
              </TooltipTrigger>
            </Tooltip>
            ...
            <DropdownMenuContent align="start">
              ...
              <DropdownMenuItem
                onClick={() => {
                  ...
                  if (selectedNode?.id) {
                    window.open(
                      `/map/result?paperId=${selectedNode.id}`,
                      "_blank",
                    );
                  }
                }}
  ...
  const colorItems = colorValues.map((value) => {
    ...
    return (
      <div key={value} className="flex items-center mb-2">
        <div
          className="w-4 h-4 rounded-full mr-2 border-[0.75px] border-black"
  ...
  const yearCircles = yearItems.map((item) => {
    return (
      <div key={item.year} className="flex items-center mb-2">
        <div
          className="rounded-full mr-2 border-[0.75px] border-black"
          style={{ width: `${item.diameter}px`, height: `${item.diameter}px` }}
        ></div>
        ...
        <div className="flex flex-col gap-2 items-start">
          <div className="flex gap-2 items-start">
            <div className="flex flex-col items-start justify-center mr-4">
              <h3 className="text-xs font-medium mb-2 w-16">
          ...
          <p className="text-xs text-muted-foreground">
            Scroll to zoom | ⌘ click to pan or drag
          </p>
        </div>
      </PopoverContent>
    </Popover>
