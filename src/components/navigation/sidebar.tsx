  ...
  return (
    ...
    >
      <div className="flex flex-col justify-start gap-2">
        <BoxIcon
          className={` ${isSidebarOpen ? "h-8 w-8" : "h-6 w-6"} transition-all duration-300 text-slate-200 ml-3 mt-3`}
        />
        <nav className="mt-6">
            ...
            <li>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent
                    side="right"
                    className={`${isSidebarOpen ? "hidden" : "block"} text-sm`}
                  >
                    Dashboard
                  </TooltipContent>
                </Tooltip>
                ...
                {isLoading ? (
                  ...
                  >
                    {(recentSearches ?? []).slice(0, 5).map((search) => (
                      ...
                      >
                        <span
                          className={`group-hover:text-gray-200 ${isActive(`/search?query=${search.query}`) ? "text-slate-200" : "text-slate-400"} text-[13px] font-regular truncate`}
                        >
                          {search.query}
                        </span>
                      </Link>
        ...
        <nav
          className={`mt-8 transition-all duration-300 ${isSidebarOpen ? "opacity-100" : "opacity-0"}`}
        >
          ...
          <ul className="space-y-2">
            <li>
              <Link
                ...
                href="#"
              >
                <kbd className="bg-slate-900 border border-slate-800 px-2 py-1 rounded text-slate-500 text-[11px] font-mono">
                  T
                </kbd>
                <span
                  className={`text-slate-400 group-hover:text-gray-200 text-[13px] font-medium transition-all duration-300 ${isSidebarOpen ? "opacity-100 w-full" : "opacity-0 max-w-0"}`}
                >
                  Tailwind Bio
                </span>
              </Link>
              ...
              <DropdownMenu>
                <TooltipTrigger asChild>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      className={`group flex justify-start items-center gap-3 p-2 rounded hover:bg-slate-900 data-[state=open]:bg-slate-900 data-[state=open]:text-slate-200 ${isSidebarOpen ? "opacity-100" : "opacity-0 w-0"}`}
                    >
