...
export function PageHeader({ isSidebarOpen }: { isSidebarOpen: boolean }) {
  ...
  const isAlternateRoute = alternateRoutes.some((route) =>
    pathname.startsWith(route),
  );

  return (
    <header
      className={`fixed top-0 z-[9] h-max transition-all duration-300 ${
        isSidebarOpen
      ...
      <div className="flex items-center w-full p-2 bg-background shadow-sm border-b">
            ...
            <SheetContent side="left" className="w-64 bg-slate-950 border-0">
              <div className="w-full">
                <div className="flex flex-col justify-start gap-2 w-full">
                  ...
                  <nav className="mt-12">
                    <ul className="space-y-2">
                      <li>
                        ...
                        >
                          <HomeIcon
                            className={`w-5 h-5 group-hover:text-slate-200 ${isActive("/") ? "text-slate-200" : "text-slate-400"}`}
                          />
                          <span
                            className={`group-hover:text-gray-200 ${isActive("/") ? "text-slate-200" : "text-slate-400"} text-[15px] font-medium`}
                          >
                            Dashboard
                          </span>
                        </Link>
                      ...
                      <li>
                        ...
                        >
                          <SearchIcon
                            className={`w-5 h-5 group-hover:text-slate-200 ${isActive("/search") ? "text-slate-200" : "text-slate-400"}`}
                          />
                          <span
                            className={`group-hover:text-gray-200 ${isActive("/search") ? "text-slate-200" : "text-slate-400"} text-[15px] font-medium`}
                          >
                            Search
                          </span>
                        </Link>
                      ...
                      <li>
                        ...
                        >
                          <RouteIcon
                            className={`w-5 h-5 group-hover:text-slate-200 ${isActive("/map") ? "text-slate-200" : "text-slate-400"}`}
                          />
                          <span
                            className={`group-hover:text-gray-200 ${isActive("/map") ? "text-slate-200" : "text-slate-400"} text-[15px] font-medium`}
                          >
                            Map
                          </span>
                        </Link>
                      ...
                      <li>
                        ...
                        >
                          <FileBarChart
                            className={`w-5 h-5 group-hover:text-slate-200 ${isActive("/reports") ? "text-slate-200" : "text-slate-400"}`}
                          />
                          <span
                            className={`group-hover:text-gray-200 ${isActive("/reports") ? "text-slate-200" : "text-slate-400"} text-[15px] font-medium`}
                          >
                            Reports
                          </span>
                        </Link>
                  ...
                  <nav className="mt-8">
                    <h3 className="text-[13px] text-slate-400 font-medium tracking-wide pl-3 pb-2">
                      Your teams
                    </h3>
                    <ul className="space-y-2">
                      <li>
                        <Link
                          ...
                          href="#"
                        >
                          <kbd className="bg-slate-900 border border-slate-800 px-2 py-1 rounded text-slate-500 text-[10px] font-mono">
                            T
                          </kbd>
                          <span className="text-slate-400 group-hover:text-gray-200 text-sm font-medium">
                            Tailwind Labs
                          </span>
                        </Link>
                      </li>
                  ...
                  <div className="mt-8 w-full">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
                          className="w-full group flex justify-start items-center gap-3 py-2 px-3 rounded hover:bg-gray-900 data-[state=open]:bg-gray-900"
                        >
                          <SettingsIcon className="w-5 h-5 text-slate-400 group-hover:text-slate-200" />
                          <span className="text-slate-400 group-hover:text-gray-200 font-medium">
                            Settings
                          </span>
                        </Button>
                      </DropdownMenuTrigger>
        ...
        <div className="hidden sm:flex gap-4 w-full items-center justify-end ml-2">
          ...
          <div className="flex gap-4 w-max items-center justify-end">
            ...
            {!session?.data?.user && (
              <Button
                variant={session?.data?.user ? "ghost" : "default"}
                className="data-[state=open]:bg-accent px-3"
              >
