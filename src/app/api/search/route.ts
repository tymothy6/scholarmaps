...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (existingQuery) {
      ...
      if (
        existingQuery.searchResponse &&
        existingQuery.searchResponse.data.length > 0
      ) {
        ...
        console.log(
          `Search query "${urlQuery}" exists in the database, but has no stored data. Refetching...`,
        );
      }
    }
    ...
    const queryParams = new URLSearchParams({
      ...
      fields:
        "paperId,url,title,abstract,year,referenceCount,citationCount,influentialCitationCount,tldr,journal,authors,publicationTypes,isOpenAccess,openAccessPdf",
    });
    ...
    if (response.ok) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: "Failed to save search query to database." },
          { status: 500 },
        );
      }
    } else {
