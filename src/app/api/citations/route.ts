...
import {
  ...
  PaperCitationResult,
} from "@/app/(main)/map/page";

async function fetchCitations(
  paperId: string,
  offset = 0,
  allCitations: PaperCitationResult[] = [],
): Promise<PaperCitationResult[]> {
  const limit = 1000;
  const fields =
    "isInfluential,url,title,abstract,year,referenceCount,citationCount,influentialCitationCount,journal,authors,publicationTypes";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: "Failed to fetch data from citations API" }),
      {
        status: 500,
        headers: {
          "Content-Type": "application/json",
