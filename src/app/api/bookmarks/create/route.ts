...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existingPaperIds = existingBookmarks.map(
      (bookmark: SearchPaperResult) => bookmark.paperId,
    );
    const newPaperIds = paperIds.filter(
      (paperId) => !existingPaperIds.includes(paperId),
    );
    ...
    return NextResponse.json(
      { message: "Failed to add bookmarks." },
      { status: 500 },
    );
  }
}
