...
function transformGraphData(
  citations: PaperCitationResult[],
  seedPaperData: SeedPaperData,
) {
  ...
  const validCitations = citations.filter(
    (citation) =>
      citation.citingPaper.paperId && citation.citingPaper.paperId.trim(),
  );
  ...
  const referenceCounts = citations.map(
    (citation) => citation.citingPaper.referenceCount,
  );
  const minReferenceCount = Math.min(...referenceCounts);
  const maxReferenceCount = Math.max(...referenceCounts);

  const citationCounts = citations.map(
    (citation) => citation.citingPaper.citationCount,
  );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({
        error: "Route handler failed to transform citation data",
      }),
      {
        status: 500,
        headers: {
