...
interface FlowContextTypes {
  ...
  saveStateMutation: UseMutationResult<
    void,
    unknown,
    { name: string; nodes: FlowNode[]; edges: Edge[] },
    unknown
  >;
  flowName: string;
...
const FlowContext = React.createContext<FlowContextTypes | undefined>(
  undefined,
);
...
const loadReactFlowState = async (name: string) => {
  const response = await fetch(
    `/api/reactflow/load?name=${encodeURIComponent(name)}`,
  );
...
const fetchReactFlowStates = async () => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch list of ReactFlow states: ${response.statusText}`,
    );
  }
...
const saveReactFlowState = async (
  name: string,
  nodes: FlowNode[],
  edges: Edge[],
) => {
...
export function FlowProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: reports = [] } = useQuery({
    // initialize reports with an empty array
    queryKey: ["reports"],
  ...
  const saveStateMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error.response && error.response.status === 409) {
        toast.error(
          "A report with the same name already exists. Please choose a different name.",
        );
      } else {
  ...
  const loadData = async (dataName: string, name: string) => {
    ...
    if (dataName === "initial") {
      ...
      toast.success(
        "Example report loaded successfully. Changes will not be saved.",
      );
    }
  };
