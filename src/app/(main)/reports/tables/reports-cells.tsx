  ...
  if (isDesktop) {
    return (
      <div className="pl-0 pr-2 py-2 relative">
        ...
        {result.bookmarked && (
          <BookmarkIcon className="h-4 w-4 absolute top-2 right-0" />
        )}
      </div>
    );
  }
      ...
      <div className="pl-0 pr-2 py-0">
        <Drawer open={open} onOpenChange={setOpen} shouldScaleBackground>
          <DrawerTrigger asChild>
            <Button
              variant="link"
              className="w-56 h-max whitespace-normal justify-start focus-visible:ring-0"
            >
          ...
          <DrawerContent className="flex flex-col max-h-[90%]">
            <DrawerHeader>
              ...
              <p className="text-left text-sm font-medium">
                ...
                {typeof result.year === "number" ? result.year : "N/A"})
              </p>
              <div className="flex flex-wrap gap-2 items-center justify-start">
                {result.publicationTypes &&
                  result.publicationTypes.map((publicationType, index) => (
            ...
            <div className="flex flex-col gap-2 px-4 items-start w-full">
              <Popover>
                <PopoverTrigger>
                  <Badge
                    variant="default"
                    className="mr-2 w-max font-hubotSans"
                  >
                    tl;dr
                  </Badge>
                </PopoverTrigger>
                <PopoverContent
                  side="right"
                  className="max-w-[200px] p-2 bg-primary text-primary-foreground border-primary"
                >
                  <p className="text-sm">
                    TLDRs (too long; didn&apos;t read) are short summaries
                    generated by the{" "}
                    <a
                      href="https://arxiv.org/abs/2004.15011"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="underline-offset-2 underline font-medium"
                    >
                      SciTLDR
                    </a>{" "}
                    model.
            ...
            <DrawerFooter>
              ...
              <Drawer nested={true}>
                <DrawerTrigger asChild>
                  <Button variant="default">View abstract</Button>
                </DrawerTrigger>
                  ...
                  <div className="px-4 max-h-[50vh] overflow-y-scroll">
                    <DrawerDescription>
                      {typeof abstract === "string"
                        ? abstract
                        : "🥹 Abstract not available from Semantic Scholar API."}
                    </DrawerDescription>
                  </div>
                  <DrawerFooter>
                    <DrawerClose asChild>
      ...
      <DrawerContent className="flex flex-col">
        ...
        <div className="px-4 max-h-[50vh] overflow-y-scroll">
          <DrawerDescription>
            {typeof abstract === "string"
              ? abstract
              : "Failed to load abstract from Semantic Scholar."}
          </DrawerDescription>
        </div>
        <DrawerFooter>
          <DrawerClose asChild>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem onClick={() => copyToClipboard(result.paperId)}>
          <FileSearchIcon className="mr-2 h-4 w-4" />
          Copy paper ID
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => copyToClipboard(result.url)}>
          <Link2Icon className="mr-2 h-4 w-4" />
          Copy paper URL
        </DropdownMenuItem>
        {result.bookmarked === false && (
          <DropdownMenuItem onClick={() => handleCreate(result.paperId)}>
            <BookmarkPlusIcon className="mr-2 h-4 w-4" />
            Bookmark
          </DropdownMenuItem>
        )}
        {result.bookmarked && (
          <DropdownMenuItem onClick={() => handleDelete(result.paperId)}>
