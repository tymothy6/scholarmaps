...
function PaperSearchNode() {
  ...
  const fetchSearchResults = async (paperId: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Relevance API responded with status code ${response.status}: ${response.statusText}`,
        );
      }
  ...
  return (
    <div className="shadow-md bg-background border-y px-4 py-2 flex flex-col items-center">
      <div className="flex w-full">
        <h3 className="text-sm font-semibold">Search Semantic Scholar</h3>
      </div>
      <Form {...searchForm}>
        <form
          onSubmit={searchForm.handleSubmit(onSubmit)}
          className="flex flex-row space-x-2 items-end justify-center w-full"
        >
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input
                    placeholder="Paper title, author, DOI..."
                    type="search"
                    disabled={isPending}
                    {...field}
                    className="w-56"
          ...
          <Button size="icon" className="mt-1">
            ...
            <SearchIcon
              className={`w-4 h-4 ${isLoading ? "hidden" : "block"}`}
            />
          </Button>
        </form>
      </Form>
