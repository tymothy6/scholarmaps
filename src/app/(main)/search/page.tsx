...
export async function generateMetadata({
  ...
  const title = searchParams["query"]
    ...
    : "Search";
  const description =
    "Search for papers in the Semantic Scholar research corpus";
...
async function getRecentSearches(): Promise<RecentSearchResponse[]> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Recent searches API responded with status code ${response.status}: ${response.statusText}`,
      );
    }
...
export default async function Search({ searchParams }: SearchProps) {
  ...
  return (
    <section className="p-4 lg:p-8 flex flex-col gap-2">
      <h1 className="mt-2 lg:mt-0 text-xl lg:text-2xl font-semibold lg:font-bold">
        Search{" "}
        {searchQuery ? (
          <span>
            for
            <span className="ml-2 px-2 py-1 text-lg lg:text-xl border bg-secondary rounded font-mono">
              {searchQuery}
            </span>
          </span>
        ) : (
