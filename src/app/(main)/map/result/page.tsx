...
import {
  ...
  SearchTableSkeleton,
} from "../../search/search-skeleton";
import { columns } from "./tables/citations-columns";
import { CitationResultTable } from "./tables/citations-table";
...
const CitationGraph = dynamic(
  () => import("@/components/graph/force-citations"),
  { ssr: false },
);
...
export async function generateMetadata({
  ...
  const title = searchParams["paperId"]
    ? `Map for ${searchParams["paperId"]}`
    : "Map results";
  const description =
    "Map connected papers in the Semantic Scholar research corpus";
...
export default async function Results({ searchParams }: SearchProps) {
  ...
  async function getCitationGraphData(): Promise<CitationGraphData> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Graph API responded with status code ${response.status}: ${response.statusText}`,
        );
      }
  ...
  async function PaperCitationResultsGraph() {
    ...
    return (
      <CitationGraph
        graphData={graphData}
        seedPaperId={seedPaperResponse.paperId}
      />
    );
  }

  async function PaperCitationResultsCard() {
    return <CitationResultTable columns={columns} data={results} />;
  }
