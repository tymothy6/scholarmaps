...
export interface PaperCitationResponse {
  ...
  offset: number;
  next: number;
  data: PaperCitationResult[];
}

export async function getAllPaperCitations(
  searchPaperId: string,
): Promise<PaperCitationResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Citations API responded with status code ${response.status}: ${response.statusText}`,
      );
    }
...
export async function getInfluentialPaperCitations(
  searchPaperId: string,
): Promise<PaperCitationResponse> {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";
  const queryParams = new URLSearchParams({ paperId: searchPaperId });
  ...
  try {
    ...
    const filteredData = data.filter(
      (item: PaperCitationResult) => item.isInfluential,
    );
      ...
      <div className="flex flex-col space-y-8 items-center mx-auto">
        <div className="flex flex-col space-y-4 items-center w-full md:mx-auto">
          <h2 className="text-base md:text-lg font-semibold">
            To get started, enter a paper identifier
          </h2>
          <PaperSeedSearch />
        </div>
